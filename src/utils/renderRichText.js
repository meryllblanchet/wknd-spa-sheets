...
function defaultRenderText(node, format) {
  ...
  if (node.format?.variants?.length > 0) {
    ...
    return node.format.variants.reduce((previousValue, currentValue) => format[currentValue]?.(previousValue) ?? null, node.value);
  }
...
function renderNodeList(childNodes, options) {
  if (childNodes && options) {
    return childNodes.map((node, index) => addKeyToElement(renderNode(node, options), index));
  }
 ...
 * by passing in an `options` object that may contain overrides for `nodeMap`,
 * `textFormat` and `headerStyle`
